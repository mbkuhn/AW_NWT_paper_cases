...
@app.cell
def _(np, path):
    Dfinal_three_levels = np.genfromtxt(path + '/post_processing/half_dom_3_ref_levels/sampling16000_fs.txt', skip_header=2, delimiter='')
    Dfinal_wholedom_fivelevs = np.genfromtxt(path + '/post_processing/5_ref_levels/sampling32000_fs.txt', skip_header=2, delimiter='')
...
def _(np, path):
    ## Load the CFD data
    CFDE_array256_threelev = np.genfromtxt(path+"/post_processing/half_dom_3_ref_levels/we00000.txt",skip_header=1,delimiter='')
...
def _(np, path):
    ## Load the CFD data
    CFDE_array256_wholedom_fivelev = np.genfromtxt(path+"/post_processing/5_ref_levels/we00000.txt",skip_header=1,delimiter='')
    CFDE_array256_wholedom_fivelev = np.append(CFDE_array256_wholedom_fivelev, np.genfromtxt(path+"/post_processing/5_ref_levels/we08150.txt",skip_header=1,delimiter=''), axis=0)
    CFDE_array256_wholedom_fivelev = np.append(CFDE_array256_wholedom_fivelev, np.genfromtxt(path+"/post_processing/5_ref_levels/we09000.txt",skip_header=1,delimiter=''), axis=0)
    CFDE_array256_wholedom_fivelev = np.append(CFDE_array256_wholedom_fivelev, np.genfromtxt(path+"/post_processing/5_ref_levels/we14950.txt",skip_header=1,delimiter=''), axis=0)
    CFDE_array256_wholedom_fivelev = np.append(CFDE_array256_wholedom_fivelev, np.genfromtxt(path+"/post_processing/5_ref_levels/we22150.txt",skip_header=1,delimiter=''), axis=0)
...
def _(
    ...
    plt.plot(CFD_time256_wholedom_fivelev, savgol_filter(CFDE_mech256_wholedom_fivelev, 11, 3) / CFDE_mech256_wholedom_fivelev[0], label='AMR-Wind, $\\Delta x$ = 2.28 m (five levels)')
    plt.plot(CFD_time256_fourlev, savgol_filter(CFDE_mech256_fourlev, 11, 3) / CFDE_mech256_fourlev[0], label='AMR-Wind, $\\Delta x$ = 4.5 m (four levels)')
    plt.plot(CFD_time256_threelev, savgol_filter(CFDE_mech256_threelev, 11, 3) / CFDE_mech256_threelev[0], label='AMR-Wind, $\\Delta x$ = 4.5 m (three levels)')
    plt.xlim(0, 1600)
